...
if ! code --list-extensions 2>/dev/null | grep -q "schmidt-embedded-systems.Schmidt-AI-Coder"; then
  code --install-extension schmidt-embedded-systems.Schmidt-AI-Coder &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Schmidt AI Coder evals database... "
...
read -p "💻 Do you want to build a new version of the Schmidt AI Coder extension? [currently $current_version] (y/N): " build_extension
